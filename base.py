...
class AbstractULDSampler(object):
    """ Abstract Langevin Monte Carlo sampler base class. """

    def __init__(self, energy_func, L ,energy_grad=None, prng=None,
    ...
    def simulate_dynamic(self, dt, pos, vel):
        """
        Simulate ULD dynamics from a given state and return new state.
        ...
        pos : vector
            Initial position state x.
        vel : vector
            Initial velocity.
        L: scalar
            Upper bound of Hessian
        ...
        """
        L = self.L
        gradient = self.energy_grad(pos)
        mean_v = np.exp(-2*dt)*vel - 1.0/(2*L)*(1-np.exp(-2*dt))*gradient
        mean_x = pos + 0.5*(1-np.exp(-2*dt))*vel-1.0/(2*L)*(dt-0.5*(1-np.exp(-2*dt)))*gradient
        uleft = np.eye(len(pos))*1.0/L*(dt - 0.25 * np.exp(-4*dt)-0.75 + np.exp(-2*dt))
        uright = np.eye(len(vel))*1.0/(2*L)*(1+np.exp(-4*dt)-2*np.exp(-2*dt))
        bright = np.eye(len(vel))*1.0/L * (1-np.exp(-4*dt))
        cov = np.block([
            [uleft, uright],
            [uright, bright]
        ])
    ...
    def get_samples(self, pos, dt, n_sample, burn_in=100, sample_gap = 10):
        ...
        """
        n_dim = pos.shape[0]
        pos_samples, vel_samples = np.empty((2, n_sample, n_dim), self.dtype)
        # cache = {}
        pos_samples[0], vel_samples[0] = pos, np.zeros(n_dim)
